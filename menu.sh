...
while true; do
    ...
    echo " [3] Backup Database (Docker)     (backup-db.sh)"
    echo " [4] Backup Database (Remote)     (backup-live.sh)"
    echo " [5] Restore Database             (restore-db.sh)"
    echo " [6] Sync Live to Local           (sync-db.sh)"
    echo " [7] Remove Database Stack        (remove-db.sh)"
    echo " [0] Exit"
    echo "--------------------------------------------------------------------------------"
    read -p " Pilih opsi [0-7]: " OPTION
